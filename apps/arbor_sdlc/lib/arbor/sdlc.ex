defmodule Arbor.SDLC do
  ...
  def handle_new_file(path, content, hash) do
    # Skip INDEX.md files — they're generated by ConsistencyChecker, not work items
    if Path.basename(path) == "INDEX.md" do
      :ok
    else
      handle_new_work_item(path, content, hash)
    end
  end
  ...
  def handle_changed_file(path, content, hash) do
    ...
    if Path.basename(path) == "INDEX.md" do
      ...
      handle_changed_work_item(path, content, hash)
    end
  end
